...
| A29 | 2026-07-20 | What grade points does a letter grade entered from the roster select carry? No grade-scale table exists. | Standard 4.0 scale hardcoded in `records::grades::standard_grade_points` (A=4.0 … F=0.0; I and custom codes carry none, so they never satisfy a prerequisite). An explicit `grade_points` form/API field still wins. | Prerequisite checks read `grade_points` from published grades, so the select must supply them; the standard scale is the boring default, and a per-institution scale table is cheap to add when an institution actually needs one. |
| A30 | 2026-07-21 | What timezone do registrar-entered term windows use? The forms take `datetime-local` values with no zone, and no institution-timezone entry UI exists. | Form values are read as UTC and every label says "(UTC)". Storage is UTC either way (CLAUDE.md §3). | Honest and unambiguous today; institution-timezone display/entry is a bounded later refinement (`institution.timezone` already exists in settings) that changes only the HTTP parse/format layer, not storage. |
| A31 | 2026-07-21 | Does academic standing (suspended/probation/…) block registration by itself? Unspecified; enrollment never read the column. | No — standing is the recorded designation (transcripts, registrar pages); registration blocking is done exclusively by holds, and the student page says so next to the standing form. | One enforcement mechanism instead of two overlapping ones: holds already block with reasons, audit, and overrides. A registrar suspending a student places the disciplinary hold on the same page. Nothing fails open: standing grants no ability. |
| A32 | 2026-07-22 | The licensing-UI session requires "already-issued document verification links stay live" while locked — but no public document-verification endpoint exists anywhere (no route, no migration, no design-doc mention). | Nothing to exempt today; a reminder comment sits in `licensing::middleware::is_license_exempt` so the route is added to the exemption list the day the feature is built. | Building public verification (verification codes, a migration, an anonymous endpoint) is a full backend feature, not a licensing-surface slice; inventing it here would be scope creep with real security surface. The requirement is vacuously satisfied and recorded where the future implementer will trip over it. |
| A33 | 2026-07-22 | May a platform licensing admin manually flip license status on a *self-hosted* deployment? The design docs specify the signed-file flow but never forbid the hosted-mode form there. | No — `set_status` refuses on `mode = 'self_hosted'` (422, nothing written); the panel renders read-only and names signed import as the only path. | A manual flip would silently diverge from what the signature attests, making the signed file a lie. Enforced in the service (not template visibility), proven by `self_hosted_license_state_is_read_only_in_the_panel`. |
| A34 | 2026-07-22 | The demo-readiness session asks for "Playwright or equivalent" E2E tests of the four critical journeys. | The equivalent already exists and is kept: each journey is a continuous, deterministic, explicitly-seeded HTTP acceptance test over the real router, middleware stack, templates, and PostgreSQL (`login_catalog_register_and_drop_work_as_plain_forms`; `instructor_enters_officer_publishes_student_sees_published_only` + `publish_never_flips_before_the_server_commits`; `request_review_generate_download_works_as_plain_forms` + `downloads_are_authorized_and_checksum_verified`; `platform_admin_flips_the_license_end_to_end` + `locked_institution_answers_402_and_recovery_stays_reachable`). No Playwright added. | The only behavior a browser adds is the enhancement JS, which is progressive by policy — the tested JS-off floor IS the contract, and axe covers the rendered DOM of all 29 pages. A Playwright stack (Node runner, browser binaries, a second seed path) would test the same assertions slower and flakier. If real-browser automation is ever wanted, the demo seed (`src/dev/seed.sql`) is the fixture to drive it from. |

| A35 | 2026-07-23 | The demo-dataset brief asks for ~5 faculties "verified against the real university where you can". | FEA (Education & Arts), FMSS (Management & Social Sciences), FST (Science & Technology), FHS (Health Sciences) are the University of Belize's real faculties; Agriculture is a plausible fifth for the Central Farm campus, not verified. | Four of five verified from ub.edu.bz knowledge; the fifth is inert demo reference data — wrong at worst by a name, trivially editable in `seed_demo.rs`'s `COURSES` table. |
| A36 | 2026-07-23 | The demo-dataset brief asks for one section deliberately over-enrolled via a later capacity reduction. | Not seeded: the state is unrepresentable — `section_capacity` has CHECK `enrolled_count <= capacity`, and `set_section_capacity` refuses to shrink below current enrollment. The constraint was kept and the brief item recorded instead. | Weakening a correctness constraint so the UI can display an impossible state inverts the system's priorities; if over-enrollment ever becomes a real policy (registrar-approved overload), it arrives as a modeled feature with its own rules, not as a seeded data glitch. |
| A37 | 2026-07-27 | May a student re-register for a course they already completed? The docs specify prerequisites but are silent on retakes. | Deny when any published/amended grade for that course has `grade_points > 0` (new `Denial::CourseAlreadyCompleted`); a failed attempt (F, 0.0) stays retakeable. No override path yet. | Fails closed on the academic-integrity side (no accidental duplicate credit) while keeping the common real case — retaking a failed course — open. If retake-after-pass (grade forgiveness) becomes policy, it should arrive as a registrar override type with its own audit trail, not a silent allowance. Proven by `completed_courses_cannot_be_retaken_unless_failed`. |
| A38 | 2026-08-02 | "Remaining classes per semester" on My courses: the schema has no degree-plan/program-requirements table, so "what a student still must take" is unknowable. | "Remaining" = courses offered this term with at least one open section that the student is not enrolled in (`term_courses_not_enrolled`); empty list renders "registered in every course offered this term". | Honest with the data that exists and fails safe (never claims a student is "done" with their program). When program requirements arrive as modeled data, this list narrows to them — the query and template are the only two touch points. |
| A39 | 2026-08-02 | The header account menu should show "Name, Student Id, Major, Faculty, Current enrollment level etc", but people carry no faculty or class-level in the schema — only `user_account` (now with `full_name`, migration 0019) and `student_profile` (student_number, program_code, academic_status). | The menu shows exactly what the system models: name (username fallback), email, student ID, programme code, academic standing — and role labels for non-students. Faculty/class-level are omitted rather than invented. | Showing unmodeled data would mean hardcoding fiction into identity chrome. When programme→faculty or credit-level derivation becomes modeled data, the menu's `UserMenu` struct and one query are the only touch points. |
