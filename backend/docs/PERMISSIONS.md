...
| Change license status (`POST /ui/platform/institutions/{id}/license`) | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ✅ hosted mode only — on self-hosted deployments the service refuses (422): the signed file is the only authority | `platform_admin_flips_the_license_end_to_end` (allow), `non_platform_roles_cannot_touch_the_license` (institution_admin — the strongest non-platform role — denied), `self_hosted_license_state_is_read_only_in_the_panel` |
...
| Any non-exempt route while the license is locked | ❌ for everyone — browsers navigating `/ui/` pages are 303'd to the access-suspended screen, API clients and non-GET requests get the JSON 402 | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ | ❌ (must use the exempt platform UI) | `locked_institution_answers_402_and_recovery_stays_reachable`, `licensing::middleware` unit tests |
